...
The RPM spec file provided here is for Bayemcoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://bayemcoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Bayemcoin git repository](https://github.com/BayemcoinProject/Bayemcoin) but are not part of
...
Currently only `Source10` is used, for the example `bayemcoin.conf` file.
...
in the bayemcoin git repository. At present only an SVG version of the Bayemcoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the bayemcoin
...
needed when 0.12.1 is released, a proper fix is already in the Bayemcoin git
master branch.
...
idea to build Bayemcoin Core against a static version of that library compiled
...
The `bayemcoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `bayemcoin-server` package. At this point, most RPM based
...
The `bayemcoin-server` package should have SELinux support. How to properly do
...
Bayemcoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/bayemcoin/). He has been
packaging Bayemcoin for Fedora at least since 2012.
...
4. I renamed the `bayemcoin` package that contains the Qt GUI to `bayemcoin-core` as
that appears to be how the general population refers to it, in contrast to
`bayemcoin-xt` or `bayemcoin-classic`. I wanted to make sure the general population
