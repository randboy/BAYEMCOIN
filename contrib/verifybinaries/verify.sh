...
###   bayemcoincore.org and bayemcoin.org and compares them.
...
WORKINGDIR="/tmp/bayemcoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://bayemcoin.org"
HOST2="https://bayemcoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="bayemcoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'bayemcoin-',
...
if [ $? -ne 0 ]; then
   echo "bayemcoin.org failed to provide signature file, but bayemcoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "bayemcoin.org and bayemcoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Bayemcoin Core binary release signing key installed?"
   fi
