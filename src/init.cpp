 ...
 * between bayemcoind, and bayemcoin-qt and non-server tools.
*/
...
void Shutdown()
{
    ...
    RenameThread("bayemcoin-shutoff");
    ...
    FlushWallets();
#endif
    GenerateBayemcoins(false, 0, Params());
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), BAYEMCOIN_CONF_FILENAME));
    if (mode == HMM_BAYEMCOIND)
    {
#if HAVE_DECL_DAEMON
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), BAYEMCOIN_PID_FILENAME));
#endif
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/BayemcoinProject/Bayemcoin>";
    const std::string URL_WEBSITE = "<https://bayemcoin.org>";
...
void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("bayemcoin-loadblk");
...
/** Sanity checks
 *  Ensure that Bayemcoin is running in a usable environment with all
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single Bayemcoin process is using the data directory.
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", BAYEMCOIN_CONF_FILENAME)).string());
    ...
    GenerateBayemcoins(fGenerate, gArgs.GetArg("-genproclimit", DEFAULT_GENERATE_THREADS), chainparams);
