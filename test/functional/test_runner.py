...
`test/functional/test_framework/test_framework.py:BayemcoinTestFramework.main`.
...
def main():
    ...
    tmpdir = "%s/bayemcoin_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    enable_bayemcoind = config["components"].getboolean("ENABLE_BAYEMCOIND")

    if config["environment"]["EXEEXT"] == ".exe" and not args.force:
        # https://github.com/BayemcoinProject/Bayemcoin/commit/d52802551752140cf41f0d9a225a43e84404d3e9
        # https://github.com/BayemcoinProject/Bayemcoin/pull/5677#issuecomment-136646964
        print("Tests currently disabled on Windows by default. Use --force option to enable")
        sys.exit(0)

    if not (enable_wallet and enable_utils and enable_bayemcoind):
        print("No functional tests to run. Wallet, utils, and bayemcoind must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=[]):
    # Warn if bayemcoind is already running (unix only)
    try:
        if subprocess.check_output(["pidof", "bayemcoind"]) is not None:
            print("%sWARNING!%s There is already a bayemcoind process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    #Set env vars
    if "BAYEMCOIND" not in os.environ:
        os.environ["BAYEMCOIND"] = build_dir + '/src/bayemcoind' + exeext
        os.environ["BAYEMCOINCLI"] = build_dir + '/src/bayemcoin-cli' + exeext
...
class TestHandler:
    ...
    def __init__(self, num_tests_parallel, tests_dir, tmpdir, test_list=None, flags=None):
        ...
        # In case there is a graveyard of zombie bayemcoinds, we can apply a
...
class RPCCoverage():
    ...
    commands per `bayemcoin-cli help` (`rpc_interface.txt`).
